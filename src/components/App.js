...
class App extends Component {
    constructor() {
        ...
        this.modules = {
            Board, boardmatcher, dialog, enginesyncer,
    ...
    getMoveTimes() {
        console.log(this.state.moveTimes)
        return this.state.moveTimes
    }
    ...
    async newFile({playSound = false, showInfo = false, suppressAskForSave = false, continuous = false} = {}) {
        if (!suppressAskForSave && !this.askForSave()) return

        if (showInfo && this.state.openDrawer === 'info') this.closeDrawer()
        this.setMode('play')
        if (continuous) {
            this.setPlaytimes(10)
        }
    ...
    getCurrentEnginnes() {
        return this.state.attachedEngines
    }
    ...
    async generateMove({passPlayer = null, firstMove = true, followUp = false} = {}) {
        ...
        if (!firstMove && !this.state.generatingMoves) {
            ...
            this.setState({moveTimes: 0})
        }
        this.increaseMoveTimes();
